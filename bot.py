...
class DownloadProgress:
    ...
    async def update(self, data):
        ...
        # Actualizar solo si han pasado mÃ¡s de 15 segundos o si hay cambios importantes
        if current_time - self.last_update > 15 or 'force' in data:
            ...
            progress_text = (
                f"[{self.task_id}] â¬ Descargando\n"
                f"ğ {percent}%\n"
                f"â¡ {speed}\n"
                f"â±ï¸ {elapsed_str} / ETA {eta}"
            )
...
def cancel_task(task_id):
    ...
    if task_id in current_downloads:
        ...
        try:
            if process.poll() is None:  # Si el proceso aÃºn estÃ¡ en ejecuciÃ³n
                process.terminate()
...
async def download_with_ytdlp(url, download_path, custom_filename, progress_callback, task_id):
    ...
    try:
        ...
        # Registrar proceso para posible cancelaciÃ³n
        ...
        for line in process.stdout:
            if progress_callback and task_id in active_tasks and not active_tasks[task_id].cancelled:
                ...
                if match:
                    ...
                    # Actualizar solo porcentaje si estÃ¡ disponible
                    match_percent = re.search(r'(\d+\.\d+)%', line)
                    if match_percent:
                        percent = match_percent.group(1)
                        # Forzar actualizaciÃ³n solo si hay cambio significativo
...
async def start(client: Client, message: Message):
    await message.reply(
        "ğ¤ **Bot de Descargas Avanzado**\n\n"
        "EnvÃ­a un enlace para subir el archivo a Telegram\n\n"
        "Soporte para:\n"
        "- Videos (MP4, MKV, AVI, etc.)\n"
        "- Streams (M3U8, YouTube, etc.)\n"
        "- Archivos directos (ZIP, PDF, imÃ¡genes, etc.)\n\n"
        "Para renombrar: `http://ejemplo.com/archivo.mp4 | mi_archivo.mp4`\n\n"
        "Archivos >1990MB se dividirÃ¡n automÃ¡ticamente\n\n"
        "Comandos disponibles:\n"
        "/start - Muestra este mensaje\n"
        "/cancel [ID] - Cancela una tarea en progreso\n"
        "/update - Actualiza herramientas (solo propietario)\n\n"
        "â ï¸ Solo puedes tener 1 tarea activa a la vez"
    )
...
async def update_bot(client: Client, message: Message):
    ...
    if not is_owner(message.from_user.id):
        await message.reply("â Solo el propietario puede usar este comando")
        return
        
    msg = await message.reply("ğ Actualizando herramientas...")
    ...
    try:
        ...
        await client.send_document(
            ...
            document=log_file,
            caption="â Error al actualizar herramientas"
        )
        await msg.edit("â ï¸ ActualizaciÃ³n fallida. Ver log para detalles.")
...
async def cancel_command(client: Client, message: Message):
    ...
    if cancel_task(task_id):
        await message.reply(f"â Tarea {task_id} cancelada correctamente")
    else:
        await message.reply("â ï¸ ID de tarea no encontrada o ya completada")
...
async def handle_links(client: Client, message: Message):
    ...
    if user_id in user_active_tasks:
        ...
        if task_id_actual in active_tasks:
            await message.reply(
                "â ï¸ Ya tienes una tarea en curso.\n"
                f"ID de tarea actual: `{task_id_actual}`\n\n"
                "Espera a que finalice o cancÃ©lala con:\n"
    ...
    msg = await message.reply(f"[{task_id}] â¬ Iniciando descarga...")
    ...
    if not file_path or not os.path.exists(file_path):
        await msg.edit(f"[{task_id}] â Error al descargar el contenido")
        if task_id in active_tasks:
            del active_tasks[task_id]
    ...
    if file_size > MAX_DIRECT_SIZE:
        await msg.edit(f"[{task_id}] ğ¦ Archivo grande detectado ({size_mb:.2f} MB > 1990 MB). Dividiendo...")
        await split_and_upload(client, message, msg, file_path, task_id)
    else:
        await msg.edit(f"[{task_id}] â Descarga completa ({size_mb:.2f} MB)\nâ¬ï¸ Subiendo a Telegram...")
        try:
            ...
            if is_video:
                ...
                caption = f"ğ¹ {os.path.basename(file_path)}"
                if metadata:
                    size_mb = metadata['size'] / (1024 * 1024)
                    caption = (
                        f"ğ¹ {os.path.basename(file_path)}\n"
                        f"ğ¾ {size_mb:.2f} MB\n"
                        f"ğ¥ï¸ {metadata['resolution']}\n"
                        f"â±ï¸ {metadata['duration']:.2f} seg"
                    )
            ...
            await msg.edit(f"[{task_id}] â Subida completada")
        except Exception as e:
            await msg.edit(f"[{task_id}] â Error en subida: {str(e)}")
...
async def upload_progress_callback(current, total, msg, task_id):
    ...
    if current_time - upload_progress_callback.last_update[task_id] > 15:
        ...
        try:
            await msg.edit(
                f"[{task_id}] â¬ï¸ Subiendo...\n"
                f"ğ {percent:.1f}%\n"
                f"ğ¦ {current//(1024*1024)}MB/{total//(1024*1024)}MB"
            )
        except Exception:
            pass  # Ignorar errores de actualizaciÃ³n

if __name__ == "__main__":
    logger.info("â¡ Bot iniciado con Pyrofork â¡")
    app.run()
