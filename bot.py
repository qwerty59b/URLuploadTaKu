...
async def download_with_ytdlp(url, download_path, custom_filename, progress_callback, task_id):
    ...
    try:
        cmd = [
            ...
            "--format", "bestvideo+bestaudio/best",  # Seleccionar mejor calidad automáticamente
            url
        ]
...
async def start(client: Client, message: Message):
    await message.reply(
        ...
        "Envía un enlace directo a un archivo, video o stream (HTTP, HLS, DASH, etc.):\n"
        "- Videos de YouTube, Facebook, Instagram\n"
        "- Archivos MP4, MKV, AVI, etc.\n"
...
async def handle_links(client: Client, message: Message):
    """Procesa enlaces de archivos/videos/streams"""
